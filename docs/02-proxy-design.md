...
cloud-initの設定は `infra/conf/squid-cloud-init.yml` に定義する。そのYAMLの内容をBase64エンコードし、ARMテンプレートの `customData` に埋め込む。
...
その後、同じACL追加内容を `infra/conf/squid-cloud-init.yml` に反映し、YAMLをBase64エンコードした値でARMテンプレートの `customData` も更新する。これにより、既存VMへ手動反映した内容と、ARMテンプレートから新しく作成するVMの初期設定に差分が出ないようにする。
